...
# add_transaction_tag does a read-modify-write (fetch current tags, append
# one, replace the full list). Without serializing that per transaction, two
# concurrent calls adding different tags to the same transaction both read
# the same starting set and the second write silently drops the first tag.
# This only protects against races within this process, which is the
# realistic case for an MCP host dispatching tool calls concurrently; it
# cannot protect against two independent server processes racing each other.
_transaction_tag_locks: "defaultdict[str, asyncio.Lock]" = defaultdict(asyncio.Lock)
...
async def add_transaction_tag(transaction_id: str, tag_id: str) -> str:
    ...
    try:
        client = await get_monarch_client()
        async with _transaction_tag_locks[transaction_id]:
            # redirect_posted defaults to True upstream, which answers a query
            # for a pending transaction with its posted counterpart. Since this
            # is a read modify replace, that would write the twin's tag set
            # over the pending transaction's own, silently dropping tags.
            ...
            # set_transaction_tags is a full replacement, so degrading to an
            # empty list turns this documented additive operation into a wipe.
